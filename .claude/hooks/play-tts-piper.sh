...
if [[ -n "$SPEAKER_ID" ]]; then
  ...
  # Try Python piper module first (works on Apple Silicon), fall back to binary
  if command -v python3 &>/dev/null && python3 -c "import piper" 2>/dev/null; then
    echo "$TEXT" | python3 -m piper --model "$VOICE_PATH" --speaker "$SPEAKER_ID" --length-scale "$SPEECH_RATE" --sentence-silence 2.0 --output_file "$TEMP_FILE" 2>/dev/null
  else
  ...
  # Try Python piper module first (works on Apple Silicon), fall back to binary
  if command -v python3 &>/dev/null && python3 -c "import piper" 2>/dev/null; then
    echo "$TEXT" | python3 -m piper --model "$VOICE_PATH" --length-scale "$SPEECH_RATE" --sentence-silence 2.0 --output_file "$TEMP_FILE" 2>/dev/null
  else
