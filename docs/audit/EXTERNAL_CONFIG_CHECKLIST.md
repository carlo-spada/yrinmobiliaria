...
- [ ] Environment variables — **names only** (confirm presence, not values):
  - `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `NEXT_PUBLIC_SUPABASE_PROJECT_ID`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_GA_MEASUREMENT_ID`, `NEXT_PUBLIC_WHATSAPP_NUMBER`, `NEXT_PUBLIC_TURNSTILE_SITE_KEY` (site key pública de Turnstile — debe estar en **Production Y Preview**).
...
- [ ] **Turnstile (implementado en Phase 2.6):** widget creado en Cloudflare → Turnstile con hostnames `yrinmobiliaria.com`, `www.yrinmobiliaria.com`, `localhost`; modo "Managed". Site key → Vercel (`NEXT_PUBLIC_TURNSTILE_SITE_KEY`); secret → Supabase (abajo). (Bot Fight Mode adicional opcional.)
...
- [ ] Function secrets present (names only): `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `TURNSTILE_SECRET_KEY` (secreto de Turnstile), `TURNSTILE_ENFORCE` (poner `=true` para activar la verificación del captcha en los formularios; honeypot + rate-limit ya están activos sin él).
